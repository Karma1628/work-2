...
class Visa_Train(Dataset):
    ...
    def __init__(self, 
        ...
        self.image_paths = glob.glob(root_dir + '/train/good/*.JPG') 
...
class Visa_test(Dataset):
    ...
    def __init__(self, 
                 root_dir=None, 
                 resize_shape=256):
        self.image_paths = glob.glob(root_dir + '/test/*/*.JPG')
        self.mask_path = root_dir + '/ground_truth/bad/'
        self.resize_shape = [resize_shape, resize_shape]
        self.transform = transforms.Compose([transforms.ToPILImage(), 
                                             transforms.ToTensor(),
                                             transforms.Normalize(mean=[0.485, 0.456, 0.406],
    ...
    def __getitem__(self, idx):
        ...
        image_name = os.path.basename(image_path) # 000.JPG
        image_dir = os.path.basename(os.path.dirname(image_path)) # bad or good
        if image_dir == 'good':
...
if __name__ == '__main__':
    ...
    # zipper screw grid
    dataset = Visa_Train(
        root_dir = '/home/karma1729/Desktop/data/Visa_1cls/candle',
